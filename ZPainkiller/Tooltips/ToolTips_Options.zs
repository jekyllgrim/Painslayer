 ...
 */
  ...
  /*
  * FILE - ToolTips_Options.txt
  *
  * OptionMenu Descendents
  *
  * VERSION 0.3.2.1
  *
  */
...
*	PKTLTPControl
*/
class OptionMenuItemPKTLTPControl : OptionMenuItemControl
{
	String mTooltip;
	Name cr_TipColor;

	OptionMenuItemPKTLTPControl Init(String label, String tooltip, Name tipColor, Name command)
	{
		mTooltip = tooltip;
...
*	PKTLTPMapControl
*/
class OptionMenuItemPKTLTPMapControl : OptionMenuItemMapControl
{
	String mTooltip;
	Name cr_TipColor;
	
	OptionMenuItemPKTLTPMapControl Init(String label, String tooltip, Name tipColor, Name command)
	{
		mTooltip = tooltip;
		cr_TipColor = tipColor;
		Super.Init(label, command);
...
*	PKTLTPOption
*/
class OptionMenuItemPKTLTPOption : OptionMenuItemOption
{
	String mTooltip;
	Name cr_TipColor;

	OptionMenuItemPKTLTPOption Init(String label, String tooltip, Name tipColor, Name command, Name values, CVar graycheck = null, int center = 0)
	{
		mTooltip = tooltip;
		cr_TipColor = tipColor;
...
*	PKTLTPSlider
*/
class OptionMenuItemPKTLTPSlider : OptionMenuItemSlider
{
	String mTooltip;
	Name cr_TipColor;
	
	OptionMenuItemPKTLTPSlider Init(String label, String tooltip, Name tipColor, Name command, double min, double max, double step, int showval = 1)
	{
		mTooltip = tooltip;
		cr_TipColor = tipColor;
...
*	PKTLTPScaleSlider
*/
class OptionMenuItemPKTLTPScaleSlider : OptionMenuItemScaleSlider
{
	String mTooltip;
	Name cr_TipColor;
	
	OptionMenuItemPKTLTPScaleSlider Init(String label, String tooltip, Name tipColor, Name command, double min, double max, double step, String zero, String negone = "")
	{
		mTooltip = tooltip;
		cr_TipColor = tipColor;
...
*	PKTLTPColorPicker
*/
class OptionMenuItemPKTLTPColorPicker : OptionMenuItemColorPicker
{
	String mTooltip;
	Name cr_TipColor;

	OptionMenuItemPKTLTPColorPicker Init(String label, String tooltip, Name tipColor, Name command)
	{
		mTooltip = tooltip;
		cr_TipColor = tipColor;
		Super.Init(label, command);
...
*	PKTLTPTextField
*/
class OptionMenuItemPKTLTPTextField : OptionMenuItemTextField
{
	String mTooltip;
	Name cr_TipColor;
	
	OptionMenuItemPKTLTPTextField Init(String label, String tooltip, Name tipColor, Name command, CVar graycheck = null)
	{
		mTooltip = tooltip;
		cr_TipColor = tipColor;
...
*	PKTLTPNumberField
*/
class OptionMenuItemPKTLTPNumberField : OptionMenuItemNumberField
{
	String mTooltip;
	Name cr_TipColor;

	OptionMenuItemPKTLTPNumberField Init(String label, String tooltip, Name tipColor, Name command, float minimum = 0, float maximum = 100, float step = 1, CVar graycheck = null)
	{
		mTooltip = tooltip;
		cr_TipColor = tipColor;
...
class OptionMenuItemPKTLTPSubmenu : OptionMenuItemSubmenu
{
	String mTooltip;
	Name cr_TipColor;
	
	OptionMenuItemPKTLTPSubmenu Init(String label, String tooltip, Name tipColor, Name command, int param = 0, bool centered = false)
	{
		mTooltip = tooltip;
		cr_TipColor = tipColor;
...
class pktltpConsole
{
	static void Print(string str) { Console.Printf(str); }
}

/*
*	PKTLTP Option Menu
*/
class PKTLTPOptionsMenu : OptionMenu
{
	enum PKTLTPBACKGROUND
	{
		bDefault = 1,
		DoomBrick,
		DoomMarble,
	...
	override void Ticker()
	{
		...
		{
			if (mDesc.mItems[mDesc.mSelectedItem] is "OptionMenuItemPKTLTPControl")
			{
				tt = StringTable.Localize(OptionMenuItemPKTLTPControl(mDesc.mItems[mDesc.mSelectedItem]).mTooltip);
				cr = OptionMenuItemPKTLTPControl(mDesc.mItems[mDesc.mSelectedItem]).cr_TipColor;
				bValidObject = true;
			}
			else if (mDesc.mItems[mDesc.mSelectedItem] is "OptionMenuItemPKTLTPMapControl")
			{
				tt = StringTable.Localize(OptionMenuItemPKTLTPMapControl(mDesc.mItems[mDesc.mSelectedItem]).mTooltip);
				cr = OptionMenuItemPKTLTPMapControl(mDesc.mItems[mDesc.mSelectedItem]).cr_TipColor;
				bValidObject = true;
			}			
			else if (mDesc.mItems[mDesc.mSelectedItem] is "OptionMenuItemPKTLTPOption")
			{
				tt = StringTable.Localize(OptionMenuItemPKTLTPOption(mDesc.mItems[mDesc.mSelectedItem]).mTooltip);
				cr = OptionMenuItemPKTLTPOption(mDesc.mItems[mDesc.mSelectedItem]).cr_TipColor;
				bValidObject = true;
			}			
			else if (mDesc.mItems[mDesc.mSelectedItem] is "OptionMenuItemPKTLTPSlider")
			{
				tt = StringTable.Localize(OptionMenuItemPKTLTPSlider(mDesc.mItems[mDesc.mSelectedItem]).mTooltip);
				cr = OptionMenuItemPKTLTPSlider(mDesc.mItems[mDesc.mSelectedItem]).cr_TipColor;
				bValidObject = true;
			}
			else if (mDesc.mItems[mDesc.mSelectedItem] is "OptionMenuItemPKTLTPScaleSlider")
			{
				tt = StringTable.Localize(OptionMenuItemPKTLTPScaleSlider(mDesc.mItems[mDesc.mSelectedItem]).mTooltip);
				cr = OptionMenuItemPKTLTPScaleSlider(mDesc.mItems[mDesc.mSelectedItem]).cr_TipColor;
				bValidObject = true;
			}			
			else if (mDesc.mItems[mDesc.mSelectedItem] is "OptionMenuItemPKTLTPTextField")
			{
				tt = StringTable.Localize(OptionMenuItemPKTLTPTextField(mDesc.mItems[mDesc.mSelectedItem]).mTooltip);
				cr = OptionMenuItemPKTLTPTextField(mDesc.mItems[mDesc.mSelectedItem]).cr_TipColor;
				bValidObject = true;		
			}			
			else if (mDesc.mItems[mDesc.mSelectedItem] is "OptionMenuItemPKTLTPColorPicker")
			{
				tt = StringTable.Localize(OptionMenuItemPKTLTPColorPicker(mDesc.mItems[mDesc.mSelectedItem]).mTooltip);
				cr = OptionMenuItemPKTLTPColorPicker(mDesc.mItems[mDesc.mSelectedItem]).cr_TipColor;
				bValidObject = true;			
			}
			else if (mDesc.mItems[mDesc.mSelectedItem] is "OptionMenuItemPKTLTPNumberField")
			{
				tt = StringTable.Localize(OptionMenuItemPKTLTPNumberField(mDesc.mItems[mDesc.mSelectedItem]).mTooltip);
				cr = OptionMenuItemPKTLTPNumberField(mDesc.mItems[mDesc.mSelectedItem]).cr_TipColor;
				bValidObject = true;
			}	
			else if (mDesc.mItems[mDesc.mSelectedItem] is "OptionMenuItemPKTLTPSubmenu")
			{
				tt = StringTable.Localize(OptionMenuItemPKTLTPSubmenu(mDesc.mItems[mDesc.mSelectedItem]).mTooltip);
				cr = OptionMenuItemPKTLTPSubmenu(mDesc.mItems[mDesc.mSelectedItem]).cr_TipColor;
				bValidObject = true;
			}
			else
				bValidObject = false;
		...
		// Update Settings
		bShowTips = Cvar.GetCVar('pktltp_show_tips', players[consoleplayer]).GetBool();
		bTimeOut = CVar.GetCVar('pktltp_time_out', players[consoleplayer]).GetBool();
		iTimeOutVal = CVar.GetCVar('pktltp_time_out_val', players[consoleplayer]).GetInt();
		bDrawOrder = CVar.GetCVar('pktltp_draw_order', players[consoleplayer]).GetBool();
		
		bDrawBorder = CVar.GetCVar('pktltp_border_toggle', players[consoleplayer]).GetBool();
		bBorderType = CVar.GetCVar('pktltp_border_type', players[consoleplayer]).GetBool();
		iBorder_HorizontalThickness = CVar.GetCVar('pktltp_border_horizontal_thickness', players[consoleplayer]).GetInt();
		iBorder_VerticalThickness = CVar.GetCVar('pktltp_border_vertical_thickness', players[consoleplayer]).GetInt();
		cBorderColor = Color(CVar.GetCVar('pktltp_border_color', players[consoleplayer]).GetInt());
		fBorderAlpha = CVar.GetCVar('pktltp_border_alpha', players[consoleplayer]).GetFloat();
		
		bDrawBackground = CVar.GetCVar('pktltp_show_background', players[consoleplayer]).GetBool();
		bBackgroundMode = CVar.GetCVar('pktltp_background_mode', players[consoleplayer]).GetBool();
		iBackground_HorizontalPadding = CVar.GetCVar('pktltp_background_horizontal_padding', players[consoleplayer]).GetInt();
		iBackground_VerticalPadding = CVar.GetCVar('pktltp_background_vertical_padding', players[consoleplayer]).GetInt();
		
		bDisplayDelay = CVar.GetCVar('pktltp_show_delay', players[consoleplayer]).GetBool();
		bDisplaySkip = CVar.GetCVar('pktltp_delay_skip', players[consoleplayer]).GetBool();
		iDelayTime = CVar.GetCVar('pktltp_delay_tics', players[consoleplayer]).GetInt();
		
		iTipLocation = CVar.GetCVar('pktltp_tooltip_location', players[consoleplayer]).GetInt();
		iPosition_HorizontalMargin = CVar.GetCVar('pktltp_location_horizontal_margin', players[consoleplayer]).GetInt();
		iPosition_VerticalMargin = CVar.GetCVar('pktltp_location_vertical_margin', players[consoleplayer]).GetInt();
		iTxtAlignment = CVar.GetCVar('pktltp_tooltip_alignment', players[consoleplayer]).GetInt();
		iFontId = CVar.GetCVar('pktltp_tooltip_font', players[consoleplayer]).GetInt();
		sCustomFont = CVar.GetCVar('pktltp_tooltip_font_name', players[consoleplayer]).GetString();
		bFontColorOverride = CVar.GetCVar('pktltp_font_color_override', players[consoleplayer]).GetBool();
		iFontColorOverride = CVar.GetCVar('pktltp_font_color', players[consoleplayer]).GetInt();
		...
		if (bDrawBackground)
			txid_Background = TexMan.CheckForTexture(backgroundSerializer(CVar.GetCVar('pktltp_background_name', players[consoleplayer]).GetInt()), TexMan.TYPE_ANY);
