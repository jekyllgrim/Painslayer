...
Class PK_CardControl : PK_InventoryToken {	
	...
	*/
	...
	void GiveGoldAmount(int amount) {
		pk_gold = Clamp(pk_gold + amount, 0, 99990);
	}
	
	clearscope int GetGoldAmount() {
		return pk_gold;
	}
...
Class PKC_Blessing : PK_BaseSilverCard {
	...
	override void GetCard() {
		...
		if (curHealth >= 100) {
			// Note that bonushealth gets automatically added to the second argument, 
			// so, to limit given health to 150, I actually have to use 100 because 
			// bonushealth is already 50 (I know, it's really weird):
